...
SETTINGS_FILE = os.path.join(_base_path(), 'camera_settings_en.json')
...
ALL_PROPS = [
    ('brightness', VP.Brightness, 'Brightness', (0, 255, 128), '☀', '#444444', '#dddddd'),
    ('contrast', VP.Contrast, 'Contrast', (0, 255, 128), '◐', '#444444', '#dddddd'),
    ('saturation', VP.Saturation, 'Saturation', (0, 255, 128), '🎨', '#999999', '#c0392b'),
    ('hue', VP.Hue, 'Hue', (-180, 180, 0), '🌈', '#6699ff', '#ff6699'),
    ('sharpness', VP.Sharpness, 'Sharpness', (0, 255, 128), '🔍', None, None),
    ('gamma', VP.Gamma, 'Gamma', (72, 500, 100), '🔘', None, None),
    ('white_balance', VP.WhiteBalance, 'White Balance', (2000, 6500, 4600), '🌡', '#7799cc', '#ddb844'),
    ('video_backlight_compensation', VP.BacklightCompensation, 'Backlight Comp.', (0, 1, 0), '💡', '#444444', '#dddddd'),
    ('gain', VP.Gain, 'Gain', (0, 255, 0), '📶', '#444444', '#dddddd'),
    ('exposure', CP.Exposure, 'Exposure', (-13, -1, -6), '📷', '#444444', '#dddddd'),
    ('focus', CP.Focus, 'Focus', (0, 250, 0), '🎯', None, None),
    ('zoom', CP.Zoom, 'Zoom', (100, 500, 100), '🔎', None, None),
    ('pan', CP.Pan, 'Pan', (-10, 10, 0), '↔', None, None),
    ('tilt', CP.Tilt, 'Tilt', (-10, 10, 0), '↕', None, None),
]
...
class CenteredInputDialog(ctk.CTkToplevel):
    def __init__(self, parent, title, text):
        ...
        ctk.CTkButton(btn_frame, text='OK', width=80, command=self._ok,
                      fg_color=BLUE, hover_color='#5a5a6a').pack(side='left', padx=4)
        ctk.CTkButton(btn_frame, text='Cancel', width=80, command=self._cancel,
...
class PropRow(ctk.CTkFrame):
    def __init__(self, parent, prop, dev_index):
        ...
        if self._unsupported:
            ...
            ctk.CTkLabel(self, text='— Not Supported', font=ctk.CTkFont(size=12),
        ...
        ctk.CTkCheckBox(self, text='Auto', variable=self.auto_var,
...
class App(ctk.CTk):
    ...
    def _build_toolbar(self):
        ...
        ctk.CTkButton(tb, text='🔍 Scan', width=70, command=self.scan,
                      fg_color=BLUE, hover_color='#5a5a6a').pack(side='left', padx=(16, 4))
        ctk.CTkButton(tb, text='🔄 Reload', width=60, command=self._reload_current,
                      fg_color=BLUE, hover_color='#5a5a6a').pack(side='left', padx=4)
        ctk.CTkButton(tb, text='↺ Default', width=60, command=self._reset_current,
                      fg_color=GRAY, hover_color='#444444').pack(side='left', padx=4)

        self._pinned = False
        self._pin_btn = ctk.CTkButton(tb, text='📌 Pin', width=60,
        ...
        ctk.CTkLabel(tb, text='⚡ Flicker', font=ctk.CTkFont(size=11),
    ...
    def _build_combo_bar(self):
        ...
        ctk.CTkLabel(cf, text='🔗 Combos', font=ctk.CTkFont(size=12, weight='bold'),
        ...
        ctk.CTkButton(cf, text='▶ Apply', width=50, command=self._combo_apply,
                      fg_color=BLUE, hover_color='#5a5a6a').pack(side='left', padx=2, pady=6)
        ctk.CTkButton(cf, text='💾 Save', width=50, command=self._combo_save,
                      fg_color=BLUE, hover_color='#5a5a6a').pack(side='left', padx=2, pady=6)
        ...
        ctk.CTkButton(cf, text='🗑 Delete', width=50, command=self._combo_del,
                      fg_color=GRAY, hover_color='#444444').pack(side='right', padx=2, pady=6)
        ctk.CTkButton(cf, text='✏ Rename', width=60, command=self._combo_rename,
                      fg_color=GRAY, hover_color='#444444').pack(side='right', padx=2, pady=6)
    ...
    def _build_preset_bar(self):
        ...
        ctk.CTkLabel(pf, text='📋 Presets', font=ctk.CTkFont(size=12, weight='bold'),
                     text_color='#b0b0b0').pack(side='left', padx=(8, 4), pady=6)
        ...
        ctk.CTkButton(pf, text='▶ Apply', width=50, command=self._preset_apply,
                      fg_color=BLUE, hover_color='#5a5a6a').pack(side='left', padx=2, pady=6)
        ctk.CTkButton(pf, text='💾 Save', width=50, command=self._preset_save,
                      fg_color=BLUE, hover_color='#5a5a6a').pack(side='left', padx=2, pady=6)
        ...
        ctk.CTkButton(pf, text='🗑 Delete', width=50, command=self._preset_del,
                      fg_color=GRAY, hover_color='#444444').pack(side='right', padx=2, pady=6)
        ctk.CTkButton(pf, text='✏ Rename', width=60, command=self._preset_rename,
                      fg_color=GRAY, hover_color='#444444').pack(side='right', padx=2, pady=6)
        ctk.CTkButton(pf, text='📤 Export', width=50, command=self._preset_export,
                      fg_color=GRAY, hover_color='#444444').pack(side='right', padx=2, pady=6)
        ctk.CTkButton(pf, text='📥 Import', width=50, command=self._preset_import,
                      fg_color=GRAY, hover_color='#444444').pack(side='right', padx=2, pady=6)
    ...
    def _preset_save(self):
        ...
        name = CenteredInputDialog(self, 'Save Preset', 'Enter preset name:').get_input()
        if name is None:
            return
        name = name.strip() or next_name(existing_names, 'Preset')
    ...
    def _preset_rename(self):
        ...
        if not old_name:
            return
        new_name = CenteredInputDialog(self, 'Rename Preset', f'Current: {old_name}\nEnter new name:').get_input()
    ...
    def _combo_save(self):
        ...
        ctk.CTkLabel(dlg, text='Combo Name', font=ctk.CTkFont(size=12, weight='bold'),
                     text_color='#b0b0b0').pack(anchor='w', padx=12, pady=(12, 0))
        name_var = ctk.StringVar(value=next_name(existing_names, 'Combo'))
        ctk.CTkEntry(dlg, textvariable=name_var, width=300).pack(padx=12, pady=(4, 8))

        ctk.CTkLabel(dlg, text='Select preset for each camera:',
        ...
        def _ok():
            name = name_var.get().strip() or next_name(existing_names, 'Combo')
        ...
        ctk.CTkButton(dlg, text='✓ OK', width=100, command=_ok,
    ...
    def _combo_rename(self):
        ...
        if not old_name:
            return
        new_name = CenteredInputDialog(self, 'Rename Combo', f'Current: {old_name}\nEnter new name:').get_input()
        if not new_name or new_name.strip() == old_name:
            return
        new_name = new_name.strip()
        data = self._data()
